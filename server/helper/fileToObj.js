...
//census file to js object to file
async function rawToZipCodeWithCityNamesShort() {
    const fileStream = fs.createReadStream('./raw_2019_Gaz_us_cities.txt');
    ...
    console.log('done')
    console.log(zipCodeWithCityNames[33014]);
    fs.writeFileSync('./allZipCodesFilesWithCityNamesShort.js', 'module.exports.allZipCodesFilesWithCityNamesShort = ' + util.inspect(JSON.stringify(zipCodeWithCityNames)), 'utf-8')
}
...
//census file to js object to file
async function makeCityDataFileWithZipCodeArray() {
    ...
    for (zipCode in cityFiles) {
        ...
        if (cityFiles[zipCode].places) {
            for (city in cityFiles[zipCode].places) {
                ...
                if (allCities[city]) {
                    let foundCity = false
                    for (specCity of allCities[city]) {
                        if (specCity.lat === cityFiles[zipCode].places[city].location.lat && specCity.lon === cityFiles[zipCode].places[city].location.lon) {
                            specCity.zipCode.push(zipCode);
                            foundCity = true
                        }
                    }

                    if (allCities[city] && !foundCity) {
                        allCities[city].push(
                            {
                                ...cityFiles[zipCode].places[city].location,
                                state: cityFiles[zipCode].state.short,
                                zipCode: [zipCode]
                            })
                    }
    ...
    console.log(allCities["Woodbury"]);
    fs.writeFileSync('./shortCityData.js', 'module.exports.shortCityData = ' + util.inspect(JSON.stringify(allCities)), 'utf-8')
}


//rawToZipCodewithCityCodes();
//rawToZipCodeWithCityNames();
rawToZipCodeWithCityNamesShort();
//makeCityDataFile();
//makeCityDataFileWithZipCodeArray();
